...
public class SharleneCommand implements Callable<Integer> {
  ...
  @ArgGroup(heading = "%nFile Options:%n", exclusive = false, multiplicity = "1")
  ...
  public Integer call() {
    ...
    File inputFile =
        InputFileValidator.orEnsureDefaultCharlesJarFile(this.fileOptionsSection.inputFile);
    // Get the output file.
    File outputFile =
        OutputFileValidator.orAddFileNameIfIsDirectory(this.fileOptionsSection.outputFile);
    // Get the overwrite flag.
    Boolean canOverwriteOutputFile = this.fileOptionsSection.canOverwriteOutputFile;
    ...
    // Check if the output file already exists and prompt the user to confirm overwriting if
    // necessary.
    if (!canOverwriteOutputFile
